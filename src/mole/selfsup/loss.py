"""iBOT loss (DINO CLS loss + iBOT/MIM patch loss).

Faithful port of ``main_attmask.iBOTLoss``, made distributed-OPTIONAL: the teacher
center updates all-reduce only when a process group is initialized, so it runs on
a single GPU/CPU without ``torch.distributed``.
"""
...
class iBOTLoss(nn.Module):
    def __init__(self, out_dim, patch_out_dim, ngcrops, nlcrops, warmup_teacher_temp,
                 teacher_temp, warmup_teacher_temp2, teacher_temp2, warmup_teacher_temp_epochs,
                 nepochs, student_temp=0.1, center_momentum=0.9, center_momentum2=0.9,
    ...
    def update_center(self, teacher_cls, teacher_patch):
        ...
        cls_center = cls_center / (len(teacher_cls) * _world_size())
        self.center = self.center * self.center_momentum + cls_center * (1 - self.center_momentum)

        patch_center = torch.sum(teacher_patch.mean(1), dim=0, keepdim=True)
        if _dist_on():
            dist.all_reduce(patch_center)
        patch_center = patch_center / (len(teacher_patch) * _world_size())
