...
Modified to remove ICDAR2017-specific validation code for use with custom datasets.
"""
...
from docloader import icdar2017patcher
import utils
...
# Evaluation modules - commented out as they may not be present
...
def get_args_parser():
    ...
    parser.add_argument('--sanity_check', default=False, type=utils.bool_flag)
    ...
    parser.add_argument('--num_class_tokens', default=1, type=int, help="""Number of class tokens to 
                        be used. The final output is the concatenation of the class tokens.""")
    parser.add_argument('--out_dim', default=256, type=int, help="""Dimensionality of
        output for [CLS] token.""")
    parser.add_argument('--patch_out_dim', default=256, type=int, help="""Dimensionality of
    ...
    # Augmentation
    parser.add_argument('--augmentation', default='morph', type=str, choices=['ibot', 'morph'],
        help="""Set augmentation strategy (default: %(default)s)""")
    ...
    # Misc
    parser.add_argument('--backend', default='nccl', choices=['nccl', 'gloo'], type=str, help='Specify backend nccl or gloo')
    parser.add_argument('--data_path', default='/data/traven/icdar17/train_binary', type=str,
        help='Please specify path to the training data.')
    parser.add_argument('--overlap', default=0.5, type=float, help='Percent of overlap.')
    parser.add_argument('--at_least_white', default=0.00, type=float, help='Minimum amount of white pixels.')
    parser.add_argument('--window_size', default=256, type=int, help='Size of windows that are extracted from document images')
...
def train_attmask(args):
    ...
    elif args.augmentation == 'morph':
        transform_train = DataAugmentationMorph(
            args.global_crops_scale,
            args.local_crops_scale,
            args.local_crops_number,
    ...
    dataset_train = ImageFolderMask(
        ...
        at_least_white=args.at_least_white,
    ...
    args.dataset_train_len = len(dataset_train)
    print(f"Data loaded for training: there are {args.dataset_train_len} images.")
    ...
    with (Path(args.output_dir) / "config.json").open("w") as f:
        f.write(json.dumps(vars(args) | {'command': str(sys.argv), 'git': utils.get_sha(), 'start_time': start_time}, sort_keys=True, indent=2))
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            if args.pred_shape == 'rand' or args.pred_shape == 'block':
                ...
                cls_attention = teacher_attention[:, :, :args.num_class_tokens, args.num_class_tokens:].mean((1,2)).detach().clone()

                # Get AttMask
                masks = AttMask(cls_attention,
                                args.masking_prob,
                                args.pred_shape,
                                data_loader.dataset.get_pred_ratio(),
                                args.show_max * data_loader.dataset.get_pred_ratio(),
                                args.show_max)
                
                masks = [mask for mask in masks.chunk(args.global_crops_number, 0)]
            else:
                assert False, f"Unknown pred_shape: {args.pred_shape}"
...
class DataAugmentationMorph(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number, patch_size=16, drop_rate=0.5):
        self.patch_size = patch_size
        self.avg2d = torch.nn.AvgPool2d(self.patch_size, self.patch_size)
        self.n_fg = 1 / (patch_size ** 2)
